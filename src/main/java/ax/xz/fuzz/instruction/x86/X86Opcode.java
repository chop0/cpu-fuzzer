...
public final class X86Opcode extends Opcode {
	...
	public X86Opcode(EnumSet<Prefix> prefixes, String icedFieldName, String mnemonic, int icedVariant, Operand[] operands) {
		...
		var anything = new ResourcePartition(StatusFlag.all(), X86Architecture.ofNative().trackedRegisters(), MemoryPartition.of(mem), mem);
...
//		}
	...
	public static X86Opcode of(int icedVariant, String icedFieldName) {
		...
		var supportedSet = new ResourcePartition(StatusFlag.all(), X86Architecture.ofNative().trackedRegisters(), MemoryPartition.of(fakeMem), fakeMem);
