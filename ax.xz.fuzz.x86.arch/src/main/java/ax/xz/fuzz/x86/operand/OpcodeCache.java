...
import ax.xz.fuzz.x86.arch.X86UarchInfo;
...
import static ax.xz.fuzz.mman.MemoryUtils.Protection.READ;
import static ax.xz.fuzz.mman.MemoryUtils.Protection.WRITE;
...
public record OpcodeCache(int version, Opcode[] opcodes) {
	...
	private static boolean doesOpcodeWork(X86Opcode opcode) {
		try (var arena = Arena.ofConfined()) {
			...
			var rp = new ResourcePartition(StatusFlag.all(), nativeArchitecture().validRegisters(), MemoryPartition.of(scratch), scratch);
