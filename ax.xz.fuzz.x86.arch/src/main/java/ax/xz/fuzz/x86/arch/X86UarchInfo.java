...
public record X86UarchInfo(long supportedExtensions) {
	...
	public static boolean checkInstructionExistence(Consumer<CodeAssembler> instructionGen) {
		try (var arena = Arena.ofConfined()) {
			var code = mmap(arena, NULL, 4096, READ, WRITE, EXECUTE);
			code.fill((byte) 0xcc);

			var assembler = new CodeAssembler(64);
			var fault = assembler.createLabel();

			assembler.xbegin(fault);
			instructionGen.accept(assembler);
			assembler.xend();
			assembler.mov(rax, 0);
			assembler.ret();
			...
			return (long)Linker.nativeLinker().downcallHandle(code, FunctionDescriptor.of(JAVA_LONG))
				.invokeExact() == 0;
