...
static void segfault_handler(int signal, siginfo_t *si, void *unused);
void save_and_return(void);


void jump_to_code(void *code, struct saved_state *saved_state);
void test_case_finish(void);
void restore_fs_gs_base(void);
...
static int masked_signals[] = { SIGSEGV, SIGBUS, SIGILL, SIGFPE, SIGTRAP };
#define NUM_MASKED_SIGNALS (sizeof(masked_signals) / sizeof(masked_signals[0]))

static struct sigaction saved_sigactions[NUM_MASKED_SIGNALS];

static void setup_signal_handlers(void) {
    for (int i = 0; i < NUM_MASKED_SIGNALS; i++) {
        struct sigaction sa;
        sa.sa_flags = SA_SIGINFO | SA_NODEFER | SA_ONSTACK | SA_RESETHAND;
        sigemptyset(&sa.sa_mask);
        sa.sa_sigaction = segfault_handler;
        if (sigaction(masked_signals[i], &sa, NULL) == -1) {
            perror("sigaction");
            exit(EXIT_FAILURE);
        }
    }
...
static void restore_signal_handlers(void) {
    for (int i = 0; i < NUM_MASKED_SIGNALS; i++) {
        if (sigaction(masked_signals[i], &saved_sigactions[i], NULL) == -1) {
            perror("sigaction");
            exit(EXIT_FAILURE);
        }
...
static void save_signal_handlers(void) {
    for (int i = 0; i < NUM_MASKED_SIGNALS; i++) {
        saved_sigactions[i] = (struct sigaction){0};
        if (sigaction(masked_signals[i], NULL, &saved_sigactions[i]) == -1) {
            perror("sigaction");
            exit(EXIT_FAILURE);
        }
    }
