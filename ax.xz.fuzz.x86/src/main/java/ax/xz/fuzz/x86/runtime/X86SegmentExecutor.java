...
import static ax.xz.fuzz.x86.arch.X86Architecture.nativeArchitecture;
import static ax.xz.fuzz.x86.arch.x86RegisterDescriptor.*;
import static ax.xz.fuzz.x86.tester.slave_h.*;
import static java.lang.foreign.MemoryLayout.PathElement.groupElement;

public class X86SegmentExecutor implements SegmentExecutor {
	static {
		X86LibraryLoader.load();
	}

	public ExecutionResult runCode(MemorySegment code, CPUState initialState) {
		try (var arena = Arena.ofConfined()) {
			var result = execution_result.allocate(arena);
			var state = execution_result.state(result);

			var tracked = nativeArchitecture().trackedRegisters();

			storeState(tracked, initialState, state);
			...
			var map = loadState(tracked, state);
	...
	@Override
	public long okExitAddress() {
		return trampoline_return_address().address();
	}
